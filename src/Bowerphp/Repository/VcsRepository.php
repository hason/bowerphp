...
use Composer\Repository\VcsRepository as BaseVcsRepository;
 ...
 */
class VcsRepository extends BaseVcsRepository
{
    public function __construct(array $repoConfig, IOInterface $io, Config $config, EventDispatcher $dispatcher = null, array $drivers = null)
    {
        $drivers = $drivers ?: array(
            'github'        => 'Bowerphp\Repository\Vcs\GitHubDriver',
            'git-bitbucket' => 'Bowerphp\Repository\Vcs\GitBitbucketDriver',
            'git'           => 'Bowerphp\Repository\Vcs\GitDriver',
            // svn must be last because identifying a subversion server for sure is practically impossible
            'svn'           => 'Bowerphp\Repository\Vcs\SvnDriver',
        );
    ...
    protected function initialize()
    {
        ...
        try {
            if ($driver->hasBowerFile($driver->getRootIdentifier())) {
                $data = $driver->getBowerInformation($driver->getRootIdentifier());
        ...
        foreach ($driver->getTags() as $tag => $identifier) {
            ...
            try {
                if (!$data = $driver->getBowerInformation($identifier)) {
                    if ($verbose) {
